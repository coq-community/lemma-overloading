...
COQLIBS?=-I ../../Saclay/ReleasedSsreflect/src -R ../../Saclay/ReleasedSsreflect/theories Ssreflect\
  -R . LemmaOverloading
COQDOCLIBS?=-R ../../Saclay/ReleasedSsreflect/theories Ssreflect\
  -R . LemmaOverloading
...
install:
	cd "." && for i in $(VOFILES) $(CMOFILES) $(CMIFILES) $(CMAFILES); do \
	 install -d "`dirname "$(DSTROOT)"$(COQLIBINSTALL)/LemmaOverloading/$$i`"; \
	 install -m 0644 $$i "$(DSTROOT)"$(COQLIBINSTALL)/LemmaOverloading/$$i; \
	done
...
uninstall_me.sh:
	echo '#!/bin/sh' > $@ 
	printf 'cd "$${DSTROOT}"$(COQLIBINSTALL)/LemmaOverloading && rm -f $(VOFILES) $(CMOFILES) $(CMIFILES) $(CMAFILES) && find . -type d -and -empty -delete\ncd "$${DSTROOT}"$(COQLIBINSTALL) && find "LemmaOverloading" -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
